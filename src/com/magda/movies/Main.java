		...
		for(int k : K) {
			...
			for(int subsetLen = 1; subsetLen <= 18; subsetLen++) {
				...
				for(int newElement=1; newElement<=18; newElement++) {
					...
					QEstimator qEstimator = new QEstimator(fc.getComparisonResults(), k, data.getValidationSet(), data.getTrainMap());
				...
				//System.out.println("k: "+k+", sublen: "+subsetLen+", bestNewElementQ: "+bestNewElementQ +", best Q: " + bestQ);
